...
import (
	...
	"github.com/9d4/semaphore/oauth2/errors"
)
...
type Server struct {
	...
	// ConsentRoute determines the frontend path of consent screen
	ConsentRoute string
}

func (s *Server) redirectError(ctx *fiber.Ctx, req *AuthorizeRequest, err error) error {
	if req == nil {
		return err
	}
...
func (s *Server) redirect(ctx *fiber.Ctx, req *AuthorizeRequest, data map[string]interface{}) error {
	uri, err := s.GetRedirectURI(req, data)
	if err != nil {
		return err
	}

	ctx.Set(fiber.HeaderLocation, uri)
	ctx.SendStatus(fiber.StatusFound)
	return nil
}
...
func (s *Server) token(ctx *fiber.Ctx, data map[string]interface{}, header map[string]string, statusCode ...int) error {
...
// CheckResponseType check if rt allowed in s.Config.AllowedResponseTypes
...
func (s *Server) ValidationAuthorizeRequest(ctx *fiber.Ctx) (*AuthorizeRequest, error) {
	...
	if !ctx.Context().IsGet() || ctx.Context().IsPost() || clientID == "" {
		return nil, fiber.NewError(fiber.StatusBadRequest, errors.ErrInvalidRequest.Error())
	}

	resType := oauth2.ResponseType(ctx.FormValue("response_type"))
	if resType.String() == "" {
		return nil, fiber.NewError(fiber.StatusBadRequest, errors.ErrUnsupportedResponseType.Error())
...
func (s *Server) HandleAuthorizeRequest(ctx *fiber.Ctx) error {
	authReq, err := s.ValidationAuthorizeRequest(ctx)
	if err != nil {
		return s.redirectError(ctx, authReq, err)
	}
	...
	if fn := s.AccessTokenExpHandler; fn != nil {
		...
		authReq.AccessTokenExp = exp
	}

	ti, err := s.GetAuthorizeToken(authReq)
	if err != nil {
		return s.redirectError(ctx, authReq, err)
	}
	...
	if authReq.RedirectURI == "" {
		client, err := s.Manager.GetClient(authReq.ClientID)
		if err != nil {
			return err
		}
		authReq.RedirectURI = client.GetDomain()
	}

	return s.redirect(ctx, authReq, s.GetAuthorizeData(authReq.ResponseType, ti))
}

// ValidationTokenRequest the token request validation
func (s *Server) ValidationTokenRequest(ctx *fiber.Ctx) (oauth2.GrantType, *oauth2.TokenGenerateRequest, error) {
	if method := ctx.Method(); method == fiber.MethodPost || (s.Config.AllowGetAccessRequest && method == fiber.MethodGet) {
